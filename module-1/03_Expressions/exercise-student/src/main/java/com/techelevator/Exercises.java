...
public class Exercises {
	...
	public boolean sleepIn(boolean weekday, boolean vacation) {
		//if vacation is true then sleep in
		//if weekday is false then sleep in
		//if weekday is true and vacation is false then don't sleep in
		if (vacation) { //could also write if (vacation || !weekday) or return vacation || !weekday;
			return true;
		} else if (weekday != true) { //could also use (weekday == false) or (!weekday)
			return true;
		}
		return false;
	}
	...
	public boolean monkeyTrouble(boolean aSmile, boolean bSmile) {
		//Monkey a is smiling
		//Monkey b is smiling
		//If both monkeys are smiling then in trouble
		//If both monkeys are not smiling then in trouble
		if (aSmile && bSmile) {
			return true;
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
		...
		//if cost of meal < 15 then return "ginormous"
