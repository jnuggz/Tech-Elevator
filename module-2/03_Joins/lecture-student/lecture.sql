...
SELECT payment.*, customer.first_name, customer.last_name --keep everything from payment table, only display first name and last name column from Customer table
FROM payment
JOIN customer ON payment.customer_id = customer.customer_id
WHERE payment_id = 16666;
...
SELECT payment.payment_id, customer.first_name, customer.last_name, rental.return_date, film.title, 
        ARRAY_AGG(actor.first_name|| ' ' || actor.last_name) AS actor_name --this will work only by grouping alike columns into 1 row
FROM payment
JOIN customer ON payment.customer_id = customer.customer_id
JOIN rental ON payment.rental_id = rental.rental_id
JOIN inventory ON rental.inventory_id = inventory.inventory_id
