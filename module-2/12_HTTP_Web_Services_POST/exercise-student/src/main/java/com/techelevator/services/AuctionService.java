...
public class AuctionService {
    ...
    public Auction add(String auctionString) {
        ...
        try {
            addAuction = makeAuction(auctionString);
            if (addAuction != null) {
                HttpEntity entity = makeEntity(addAuction);
                restTemplate.postForObject(API_URL, entity, Auction.class);
            }
        } catch (RestClientResponseException e) {
            console.printError("No Auction added. Please try again.");
    ...
    public Auction update(String auctionString) {
        Auction updateAuction = null;
        try {
            updateAuction = makeAuction(auctionString);
            if (updateAuction != null) {
                HttpEntity<Auction> entity = makeEntity(updateAuction);
                //http://localhost:3000/auctions/{id}
                restTemplate.put(API_URL + "/" + updateAuction.getId(), entity);
            }
        } catch (RestClientResponseException e) {
            console.printError("No Auction updated. Please try again.");
        } catch (ResourceAccessException e) {
            console.printError("A network error occurred.");
        }
    ...
    public boolean delete(int id) throws RestClientResponseException, ResourceAccessException {
        ...
        try {
            ...
            console.printError("No Auction deleted. Please try again.");
        } catch (ResourceAccessException e) {
            console.printError("A network error occurred.");
        }
        return result;
