...
CREATE TABLE ourtable (
        id serial primary key, --Column_name (id), Data Type (serial). Serial is a big int, increments by 1
        name varchar(10) --Column (name), Data Type (varchar)
);

 SELECT * FROM ourtable;
  
-- Get the next value of the sequence
SELECT nextval('ourtable_id_seq'); --nextval is a method in PostGres, tells the next value in the sequence
--each time this method is called, the number increases by 1
...
INSERT INTO ourtable (id, name) VALUES ( XX , 'Kevin'); --Can manually input the id (9)

-- Try to insert another value using DEFAULT or a SubQuery or by Omitting the column in the insert
INSERT INTO ourtable (id, name) VALUES (DEFAULT, 'Vinny'); --This won't work because id (9) already exists, manually inputing id doesn't affect the sequence, must run twice to get from 8 to 10 
