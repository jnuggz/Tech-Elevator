...
function displayReviews() {
  ...
  reviews.forEach( review => {
    ...
    // Set the class review on the container div
    ...
    // Insert all of it as the last child of main
...
function addRating(parent, numberOfStars) {
  ...
  // set the class "rating" on the div
  ratingDiv.classList.add('rating');
  // For each rating (numberOfStars)
  for (let i = 0; i < numberOfStars; i++) {
      // create a new img
      const starImg = document.createElement('img');
      // set the src 
      starImg.src = 'img/star.png';
      // set the class"ratingStar" on the img
      starImg.classList.add('ratingStar');
      // append the img to the new div
      ratingDiv.appendChild(starImg);
  }
  // append the new div to the container div (parent)
